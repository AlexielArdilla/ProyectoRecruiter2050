...
function confirmarBorrar() {
	return confirm("Está seguro que desea BORRAR esta Propuesta?");
}
</script>
  </head>
...
<div class="container-fluid">
			...
			<h2 style="color:white">${exito}</h2>
			 ...
			 <form action="ControladorBajaPropuesta" method="post" onSubmit="return confirmarBorrar()">
 
    <label for="nombre">Título Propuesta:</label>
   <select class="form-control" id="sel1" name="propuesta_id" required="">
       <c:forEach items="${propuestas}" var="propuesta">
        <option value="${propuesta.id}">${propuesta.cargo}</option>
       </c:forEach>
      </select>
