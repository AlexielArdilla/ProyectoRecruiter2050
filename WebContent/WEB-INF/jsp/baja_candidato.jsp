...
function confirmarBorrar() {
	return confirm("Está seguro que desea BORRAR este Candidato?");
}
</script>
...
<div class="container-fluid">
			 ...
			 <form action="ControladorBajaCandidato" method="post" onSubmit="return confirmarBorrar()">
 				<h2 style="color:white">${exito}</h2>
    <label for="nombre">Candidato a dar de baja:</label>
   <select class="form-control" id="estado" name="baja" required="">
        <c:forEach items="${candidatos}" var="candidato">
        <option name="id" value="${candidato.id}">${candidato.nombre_apel}</option>
        </c:forEach>
      </select>
